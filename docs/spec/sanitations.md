...
4. Added `x-ballerina-event-identifier.batched: true`, since CloudEvents delivers a top-level JSON array (potentially containing events for more than one QuickBooks company in a single request) rather than one event per request.
